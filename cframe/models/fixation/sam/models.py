...
import numpy as np
from cframe.models.fixation.sam.dcn_resnet import dcn_resnet
from .config import *
...
def upsampling(x):
    return UpSampling2D(size=(upsampling_factor, upsampling_factor), interpolation='bilinear')(x)
...
def sam_resnet(x):
    ...
    # print(dcn.output.shape)
    conv_feat = Convolution2D(512, kernel_size=3, strides=1, padding='same', activation='relu')(dcn.output)
    ...
    outs = Convolution2D(1, kernel_size=1, padding='same', activation='relu')(conv_feat)
    # print(outs.shape)
